...
def make_windows(full_window, max_size=1e6, fasta=None):
    ...
    if reg.size > max_size:
        ...
        return [
            Region(
                f"{reg.chromosome}:{reg.begin + i * chsize}-{reg.begin + (i + 1) * chsize}"
            )
...
def get_args():
    parser = ArgumentParser(
        description="plotting nanopolish methylation calls or frequency"
    )
    parser.add_argument(
        "-v",
        "--version",
    ...
    parser.add_argument(
        "--example", action="store_true", help="Show example command and exit."
    )
    parser.add_argument(
        "-o",
        "--outfile",
...
def print_example():
    ...
    meth_freq = pkg_resources.resource_filename(
        "methplotlib", "examples/meth_freq.tsv.gz"
    )
    bed = pkg_resources.resource_filename(
        "methplotlib", "examples/DNAse_cluster.bed.gz"
    )
    annotation = pkg_resources.resource_filename(
        "methplotlib", "examples/g38_locus.gtf.gz"
    )
...
def file_sniffer(filename):
    ...
    if len(header.split("\t")) == 14:  # file generated by `modbam2bed --extended`
        return "bedmethyl_extended"  # there is no header, but the file is tab separated and has 14 fields
