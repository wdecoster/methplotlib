...
# header
print("\t".join(["chromosome", "start", "end", "num_cpgs_in_group", "called_sites",
...
for key in sorted_keys:
    if sites[key].called_sites > 0:
        ...
        print("%s\t%s\t%s\t%d\t%d\t%d\t%.3f\t%s" % (
            c, s, e, sites[key].group_size, sites[key].called_sites, sites[key].called_sites_methylated, f, sites[key].sequence))
